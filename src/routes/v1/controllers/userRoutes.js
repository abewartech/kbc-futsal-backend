...
const user = {
  registerUser: (req, res, next) => {
    req.checkBody("name", "Nama field cannot be empty").notEmpty();
    req
      .checkBody("email", "Email field cannot be empty")
      .trim()
      .notEmpty();
    ...
    if (errors) {
      ...
      passport.authenticate("signup", (err, user, info) => {
        ...
        if (user) {
          ...
          res.status(200).json({
            success: false,
            message: req.flash(info).registerMessage
          });
        }
      })(req, res, next);
    }
    ...
    if (errors) {
      ...
      passport.authenticate("login", (err, user, info) => {
        ...
        if (user) {
          ...
          res.status(200).json({
            success: false,
            message: req.flash(info).loginMessage
          });
        }
      })(req, res, next);
    }
